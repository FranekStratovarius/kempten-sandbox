...
template<class T>
void t_insertionSort(T* a,int size,BinaryRelation<T,T> &binRelation){
	for (int i=1;i<size;i++){
		int j=i;
		while(j>0 && binRelation.inRelation(a[j],a[j-1])){
			swap(a[j],a[j-1]);
			j--;
		}
	}
...
int main() {
	...
	unsigned int* int_array=getRandomIntArrayOfSize(20);
	printArray<unsigned int>(int_array,20);
	InOrderIntRelation int_rel=InOrderIntRelation();
	t_insertionSort<unsigned int>(int_array,20,int_rel);
	//printArray<unsigned int>(int_array,20);
